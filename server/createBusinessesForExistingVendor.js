...
const createBusinessesForExistingVendor = async () => {
    try {
        // Find existing vendor
        const vendor = await Vendor.findOne({ email: 'rajesh.kumar@example.com' });
        ...
        console.log('✅ Found vendor:', vendor.name, 'with ID:', vendor._id);
        
        const vendorId = vendor._id;
        
        // Check if businesses already exist for this vendor
        const existingBusinesses = await Business.find({ vendor: vendorId });
        if (existingBusinesses.length > 0) {
            console.log(`📋 Vendor already has ${existingBusinesses.length} businesses:`);
        ...
        // Create businesses for this vendor
        ...
        console.log('\n🎉 Successfully created 5 dummy businesses!');
        console.log('\n📋 Summary:');
        ...
        console.log('4. Style Studio Beauty Salon (Beauty & Spa)');
...
// Run the function
createBusinessesForExistingVendor();