...
import { RecentActivityCard, PropertyPerformanceCard } from "@/components/DashboardComponents";
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const activeVendors = vendorsResponse?.filter(vendor => vendor.status === 'approved')?.length || 0;
      const totalViews = propertiesResponse?.reduce((sum, property) => sum + (property.views || 0), 0) || 0;
      const thisMonthProperties = propertiesResponse?.filter(property => {
      ...
      const lastMonthProperties = propertiesResponse?.filter(property => {
        const createdDate = new Date(property.createdAt);
        const currentDate = new Date();
      ...
      const propertyGrowth = lastMonthProperties > 0 ? 
        Math.round(((thisMonthProperties - lastMonthProperties) / lastMonthProperties) * 100) : 0;
